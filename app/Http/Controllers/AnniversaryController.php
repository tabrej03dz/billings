...
public function import(Request $request)
{
    ...
    $phoneKeys = ['phone', 'mobile', 'mobile_no', 'phoneno', 'contact', 'contact_no'];
    $dateKeys  = ['date_of_anniversary', 'anniversary_date', 'anniversary', 'dateofanniversary', 'doa'];
    $nameKeys  = ['name', 'full_name', 'customer_name'];
    $timeKeys  = ['wish_time', 'time', 'send_time', 'whatsapp_time'];
    ...
    try {
        foreach (array_slice($rows, 1) as $i => $row) {
            ...
            $phone = trim((string) ($row[$idxPhone] ?? ''));
            $dateRaw = $row[$idxDate] ?? null;
            $timeRaw = $idxTime !== false ? ($row[$idxTime] ?? null) : null;
            ...
            $wishTime = $this->parseExcelTime($timeRaw);

            if ($timeRaw !== null && trim((string) $timeRaw) !== '' && !$wishTime) {
                $errors[] = "Row {$rowNumber}: Invalid wish_time";
                $skipped++;
                continue;
            }
...
private function parseExcelDate($value): ?Carbon
{
    ...
    foreach (['Y-m-d', 'd-m-Y', 'd/m/Y', 'm/d/Y'] as $format) {
        try {
            return Carbon::createFromFormat($format, $v);
        } catch (\Throwable $e) {}
    }
...
private function parseExcelTime($value): ?string
{
    ...
    if (is_numeric($value)) {
        try {
            ...
            return Carbon::instance($dateTime)->format('H:i');
        } catch (\Throwable $e) {
            return null;
        }
    }
    ...
    foreach (['H:i', 'H:i:s', 'h:i A', 'h:i a'] as $format) {
        try {
            return Carbon::createFromFormat($format, $v)->format('H:i');
        } catch (\Throwable $e) {}
    }
