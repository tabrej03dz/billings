@php
    ...
    $oldItems = old('items', $isEdit ? $purchase->items->toArray() : [
        ['item_id' => null, 'qty' => 1, 'rate' => 0, 'amount' => 0],
    ]);
@endphp

<div class="space-y-6 text-gray-900 dark:text-neutral-100">
        ...
        <div>
            ...
            <select name="supplier_id"
                ...
                @foreach($suppliers as $sup)
                    <option value="{{ $sup->id }}"
                        @selected(old('supplier_id', $purchase->supplier_id) == $sup->id)>
                        {{ $sup->name }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="border rounded-lg overflow-hidden border-gray-200 dark:border-neutral-700">
        ...
        <table class="min-w-full text-xs">
            <thead class="bg-gray-50 dark:bg-neutral-900 text-gray-700 dark:text-neutral-200">
                ...
                <tr class="purchase-item-row">
                    ...
                    <td class="px-3 py-1">
                        <select name="items[{{ $i }}][item_id]"
                            ...
                            @foreach($items as $it)
                                <option value="{{ $it->id }}"
                                    @selected(($row['item_id'] ?? null) == $it->id)>
