            ...
            <div class="flex items-center gap-2 flex-wrap">
                <form method="GET" class="flex items-center gap-2">
                    <input type="text" name="search" value="{{ request('search') }}"
                           placeholder="Search name / slug / subtitle..."
            ...
            <table class="min-w-full text-sm text-left text-gray-700 dark:text-gray-300">
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-neutral-900 dark:divide-neutral-700">
                    @forelse($plans as $plan)
                        <tr>
                            <td class="px-6 py-3">{{ $plan->sort_order }}</td>

                            <td class="px-6 py-3">
                                <div class="font-medium text-gray-900 dark:text-white">{{ $plan->name }}</div>
                                <div class="text-xs text-gray-500">{{ $plan->subtitle ?: $plan->slug }}</div>
                            </td>
                            ...
                            <td class="px-6 py-3">
                                <span class="inline-flex items-center px-2 py-1 text-xs rounded bg-purple-100 text-purple-700">
                                    {{ is_array($plan->features) ? count($plan->features) : 0 }}
                                </span>
                            </td>
