<x-layouts.app :title="__('Anniversary Records')">
    <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 mb-1 bg-[#BFE0E0] dark:bg-[#354A54] p-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Anniversary Records</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Manage anniversary records.</p>
            </div>

            <a href="{{ route('anniversaries.importForm') }}"
            ...
            <a href="{{ route('anniversaries.create') }}"
        ...
        <div class="bg-slate-200 dark:bg-transparent">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-3 p-4 rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-neutral-900">
                    ...
                    @forelse ($records as $r)
                        <tr>
                            <td class="px-6 py-3 font-medium text-gray-900 dark:text-white">
                                <div class="flex items-center gap-2">
                                    ...
                                    @php
                                        ...
                                        $ann = $r->date_of_anniversary ? \Carbon\Carbon::parse($r->date_of_anniversary) : null;
                                        $next = $ann ? $ann->copy()->year($today->year) : null;

                                        if($next && $next->lt($today)) {
                                            $next->addYear();
                                        }
                            ...
                            <td class="px-6 py-3 space-x-3">
                                <a href="{{ route('anniversaries.show', $r->id) }}" class="text-blue-600 hover:underline">View</a>
                                <a href="{{ route('anniversaries.edit', $r->id) }}" class="text-yellow-600 hover:underline">Edit</a>

                                <form action="{{ route('anniversaries.destroy', $r->id) }}" method="POST" class="inline-block"
