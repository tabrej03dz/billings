<x-layouts.app :title="__('Edit User Plan')">
    <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center justify-between bg-[#BFE0E0] dark:bg-[#354A54] p-6 rounded-xl">
            ...
            <a href="{{ route('user-plans.index', ['business_id' => $userPlan->business_id]) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="business_id"
                            ...
                            @foreach($businesses as $business)
                                <option value="{{ $business->id }}"
                                    {{ old('business_id', $userPlan->business_id) == $business->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date"
                               value="{{ old('start_date', $userPlan->start_date ? \Carbon\Carbon::parse($userPlan->start_date)->format('Y-m-d') : now()->toDateString()) }}"
                               class="w-full rounded-lg border-gray-300 dark:border-neutral-700 dark:bg-neutral-800 dark:text-white">
                        @error('start_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="expiry_date" id="expiry_date"
                               value="{{ old('expiry_date', $userPlan->expiry_date ? \Carbon\Carbon::parse($userPlan->expiry_date)->format('Y-m-d') : '') }}"
                               class="w-full rounded-lg border-gray-300 dark:border-neutral-700 dark:bg-neutral-800 dark:text-white">
                        @error('expiry_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
