        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-5">
            <form method="GET" action="{{ url()->current() }}" class="grid gap-4 md:grid-cols-5 items-end">
                ...
                <div class="flex gap-2 md:col-span-2 justify-end">
                    <button type="submit"
                            class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm">
                        <i class="fas fa-filter mr-1.5 text-xs"></i> Apply Filters
                    </button>

                    <a href="{{ url()->current() }}"
                       class="inline-flex items-center px-3 py-2 rounded-lg text-xs sm:text-sm font-medium bg-slate-50 text-slate-600 hover:bg-slate-100 border border-slate-200">
                        Reset
                    </a>
                </div>
            </form>
                ...
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                    ...
                    @forelse($perUser as $index => $row)
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 text-right text-rose-500 font-medium">
                                {{ $failed }}
                            </td>
                            <td class="px-4 py-2 text-right">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] {{ $rate >= 80 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }}">
                                    {{ $rate }}%
                                </span>
                            </td>
                        </tr>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 text-xs sm:text-sm">
                    <thead class="bg-slate-50">
                    <tr>
                        <th class="px-3 sm:px-4 py-2 text-left font-semibold text-slate-500 uppercase tracking-wide text-[11px]">
                            Sent At
                        </th>
                        <th class="px-3 sm:px-4 py-2 text-left font-semibold text-slate-500 uppercase tracking-wide text-[11px]">
                            User
                    ...
                    @forelse($latestSends as $send)
                        <tr class="hover:bg-slate-50/80">
                            ...
                            <td class="px-3 sm:px-4 py-2 align-top whitespace-nowrap">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] bg-slate-100 text-slate-700 border border-slate-200">
                            ...
                            <td class="px-3 sm:px-4 py-2 align-top text-[11px] sm:text-xs">
                                @if($send->file_url)
                                    <a href="{{ $send->file_url }}" target="_blank"
                                       class="inline-flex items-center px-2 py-0.5 rounded-full border border-slate-200 text-[11px] text-slate-700 hover:bg-slate-50">
                                        View
                                    </a>
                                @else
                                    <span class="text-slate-400">—</span>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-4 py-4 text-center text-sm text-slate-500">
                                No send logs found.
                            </td>
                        </tr>
                    @endforelse
