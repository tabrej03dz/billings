<x-layouts.app :title="__('Anniversary Wish Logs')">
    <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 mb-1">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Anniversary Wish Logs</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Anniversary wish attempts ka record.</p>
            </div>
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-6 gap-3 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-neutral-900">
            ...
            <div>
                <label class="text-xs text-gray-600 dark:text-gray-400">Anniversary ID</label>
                <input type="number" name="anniversary_id" value="{{ request('anniversary_id') }}"
                       class="mt-1 w-full rounded-lg border-gray-300 dark:border-neutral-700 dark:bg-neutral-800 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                       placeholder="e.g. 10">
            </div>
                    ...
                    @forelse ($logs as $log)
                        <tr>
                            ...
                            <td class="px-6 py-3 space-x-3">
                                <a href="{{ route('anniversary-wish-logs.show', $log->id) }}"
                                   class="text-blue-600 hover:underline">View</a>

                                <form action="{{ route('anniversary-wish-logs.destroy', $log->id) }}"
                                      method="POST"
                                      class="inline-block"
                                ...
                                <form action="{{ route('anniversary-wish-logs.resend', $log->id) }}"
                                      method="POST"
                                      class="inline-block"
