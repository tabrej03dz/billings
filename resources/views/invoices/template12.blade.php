...
.page{border-left:10px solid #ef4444;padding:18px 22px}
.step{position:relative;margin-bottom:16px;padding-left:28px}
.dot{position:absolute;left:0;top:2px;width:14px;height:14px;border-radius:50%;background:#ef4444}
.stepTitle{font-size:11px;color:#ef4444;text-transform:uppercase;font-weight:700;margin-bottom:5px}
.company{font-size:26px;font-weight:700}
.doc{font-size:32px;font-weight:700;text-align:right;color:#ef4444}
table{width:100%;border-collapse:collapse}
.items th{background:#fee2e2;color:#991b1b;padding:9px;text-align:left;border-bottom:2px solid #ef4444}
.items td{padding:9px;border-bottom:1px solid #fecaca;vertical-align:top}
.text-right{text-align:right}
.desc{font-size:10px;color:#777}
.summary{width:42%;margin-left:auto}
.summary td{padding:8px;border-bottom:1px solid #fecaca}
.summary .grand td{background:#ef4444;color:#fff;font-weight:700}
.sign{text-align:right;margin-top:18px}
...
<div class="page">
    <table>
        <tr>
            ...
            <td class="doc">{{ strtoupper($type) }}</td>
        </tr>
    </table>
    ...
    <div class="step">
        ...
        No: <strong>{{ $invoiceNo }}</strong> &nbsp;&nbsp; Date: <strong>{{ $dmy($invoiceDate) }}</strong>
    </div>
    ...
    <div class="step">
        ...
        {{ $c->address ?? '-' }}<br>
        Mobile: {{ $mobile ?: '-' }} | GSTIN: {{ $gstin ?: '-' }} | PAN: {{ $pan ?: '-' }}
    </div>
