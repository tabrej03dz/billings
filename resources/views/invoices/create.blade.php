                        ...
                        <tr>
                            ...
                            <td class="px-3 py-2">
                                ...
                                <input type="text" x-model="row.description" placeholder="Description" required
                                       class="w-full border rounded px-2 py-1 border-gray-300 dark:border-neutral-700
                                              bg-white dark:bg-neutral-900 text-gray-900 dark:text-neutral-100 text-xs">
                                <p class="mt-1 text-[11px] text-gray-500" x-show="row.metal_type && row.purity">
                            ...
                            <td class="px-3 py-2">
                                <input type="number" step="0.01" min="0"
                                       x-model.number="row.stone_charges"
                                       @input="calc()"
                            ...
                            <td class="px-3 py-2">
                                <input type="number" step="0.01" min="0"
                                       ...
                                       class="w-28 border rounded px-2 py-1 border-gray-300 dark:border-neutral-700
                                              bg-white dark:bg-neutral-900 text-xs">
                                <p class="text-[11px] text-gray-500" x-show="row.metal_rate && row.metal_weight">
                                    Metal = <span
                                        x-text="(row.metal_weight * row.metal_rate).toFixed(2)"></span>
                                </p>
                            </td>
    ...
    <script>
        function invoiceForm(){
            ...
            return {
                ...
                // Metal rate helper
                // 🔹 Metal rate helper (tolerant for "24K (999)" etc.)
                findMetalRate(type, purity){
                    ...
                    // different purity patterns to try
                    const candidates = [];
                    if (pRaw) {
                        candidates.push(pRaw);                        // "24K (999)"
                        candidates.push(pRaw.split('(')[0].trim());  // "24K (999)" -> "24K"
                        candidates.push(pRaw.split(' ')[0].trim());  // "24K 999"   -> "24K"
                    }
                    ...
                    // debug (optional)
                    // console.log('metalRates', this.metalRates, 'type:', t, 'purityOptions:', purityOptions);
                ...
                // Metal weight change -> real-time recalc price from metal rate
