...
// =======================
                ...
                async saveClient(){
                    ...
                    try{
                        ...
                        this.clientId = String(data.client.id); // ✅ string
                ...
                async saveItem(){
                    ...
                    try{
                        ...
                        if(this.activeRowIndex !== null && this.items[this.activeRowIndex]){
                            this.pickItem(this.activeRowIndex, String(data.item.id));
                        }
                ...
                avgTaxPercent(){ return Number(this.avgTaxPercentRaw().toFixed(2)); },
                ...
                init(){
                    ...
                    // ✅ IMPORTANT: do prefill inside $nextTick so DOM + x-for select render properly
                    this.$nextTick(() => {

                        const cid = (INVOICE.client_id ?? INVOICE.client?.id ?? INVOICE.client?.client_id ?? '');
                        this.clientId = cid ? String(cid) : '';

                        this.hdr.date = INVOICE.invoice_date || @js($today);
                        ...
                        this.computedPrefix = INVOICE.invoice_prefix || @js($suggestedPrefix);
                        this.invoiceNo = INVOICE.invoice_number || this.invoiceNo;

                        // charges
                        let ch = INVOICE.charges_json;
                        if(typeof ch === 'string'){
                            try{ ch = JSON.parse(ch); }catch(e){ ch = []; }
                        }
                        this.charges = Array.isArray(ch) && ch.length
                            ? ch.map(x => ({ _k: Date.now()+Math.random(), name: x.name || '', amount: Number(x.amount||0) }))
                            : [ this.blankCharge() ];

                        // discount
                        this.discount.type = (INVOICE.discount_type || 'flat');
                        this.discount.value = Number(INVOICE.discount_value || 0);
                        this.ui.showDiscount = (Number(this.discount.value||0) > 0);

                        // tcs
                        this.tcs.percent = Number(INVOICE.tcs_percent || 0);
                        this.tcs.apply = (this.tcs.percent > 0);

                        // roundoff enable if stored non-zero
                        const ro = Number(INVOICE.round_off || 0);
                        this.roundOff.enabled = (ro !== 0);

                        // items
                        const invItems = Array.isArray(INVOICE.items) ? INVOICE.items : [];
                        this.items = invItems.length ? invItems.map(it => {
                            const r = this.blankRow();

                            // ✅ string id for select binding
                            r.item_id = it.item_id ? String(it.item_id) : '';

                            r.item_type = (String(it.item_type || '')).toLowerCase().trim() || null;
                            r.description = it.description || '';
                            r.hsn = it.hsn || '';
                            r.quantity = Number(it.quantity || 1);

                            r.making_rate = Number(it.making_rate || 0);
                            r.gold_purity = it.gold_purity || null;
                            r.silver_purity = it.silver_purity || null;

                            r.gold_rate = Number(it.gold_rate || 0);
                            r.silver_rate = Number(it.silver_rate || 0);
                        ...
                        // ✅ fill missing derived data (types/rates/hsn etc) + auto amount if not manual
                        this.items.forEach((r) => {
                            if(!r.item_id) return;

                            const it = this.itemsData.find(x => String(x.id) === String(r.item_id));
                            if(it){
                                if(!r.item_type) r.item_type = (String(it.type || 'product')).toLowerCase().trim();
                                if((r.tax_percent === null || r.tax_percent === undefined)) r.tax_percent = Number(it.tax_rate ?? 0);
                                ...
                                if (r.item_type === 'service') {
                                    ...
                                    if(!r.gold_rate){
                                        ...
                                        r.gold_rate = this.findMetalRate('gold', gp);
                                    }
                                    if(!r.silver_rate){
                                        const sp = r.silver_purity || (it.silver_purity ?? '999');
                                        r.silver_rate = this.findMetalRate('silver', sp);
                                    }
                                }
                            }
                        ...
                        this.payment.received = Number(INVOICE.received || 0);
                ...
                beforeSubmit(){
                    const payload = this.items.map(r => ({
                        item_id: r.item_id ? Number(r.item_id) : null, // ✅ convert back to number for backend
