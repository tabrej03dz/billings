@php
    ...
    // ✅ payment row (controller passed as $payRow)
    $payCash   = (float)($payRow->cash_amount ?? 0);
    $payOnline = (float)($payRow->online_amount ?? 0);
    $payCard   = (float)($payRow->card_amount ?? 0);
    $payCheque = (float)($payRow->cheque_amount ?? 0);

    $onlineInfo = trim(($payRow->online_mode ?? '').' '.($payRow->online_ref ?? ''));
    $cardInfo   = trim(($payRow->card_last4 ?? '').' '.($payRow->card_ref ?? ''));
    $chequeInfo = trim(($payRow->cheque_no ?? '').' '.($payRow->bank_name ?? ''));
@endphp
    <!doctype html>
<html>
<head>
    ...
    <style>
        *{ box-sizing:border-box; }
        ...
        .page{ padding:200px 18px 14px 18px; }  /* ✅ top spacing increased */
        ...
        .headbox{
            background:#f7f7f7; color:#111; padding:8px 10px 7px 10px;
            font-weight:700; letter-spacing:1px; border:1px solid #111;
        }
        ...
        .footerline{ margin-top:10px; border-top:1px solid #111; padding-top:6px; font-size:9.5px; }
    </style>
</head>
...
<div class="page">
    ...
    {{-- TOP HEADER (SIMPLE) --}}
    <table class="no-border" style="margin-bottom:10px;"> {{-- ✅ spacing --}}
        <tr class="no-border">
            <td class="no-border" style="width:25%; vertical-align:top;">
                @if(!empty($logo))
                    <img src="{{ $logo }}" style="height:44px; width:auto;" alt="Logo">
                @endif
                <div class="tiny muted" style="margin-top:3px;">Hallmark Jewellery</div>
            </td>

            <td class="no-border center" style="width:50%; vertical-align:top;">
                <div class="headbox">
                    <div style="font-size:18px;">{{ strtoupper($b->name ?? 'JEWELLERS') }}</div>
    ...
    <table class="no-border" style="margin-bottom:10px;"> {{-- ✅ spacing --}}
        <tr class="no-border">
            <td class="no-border" style="width:58%; vertical-align:top; padding-right:8px;">
                <table class="b1" style="font-size:9.5px;">
                    ...
                    <tr>
                        ...
                        <td style="padding:4px;">{{ !empty($c->gstin) ? $c->gstin : 'Unregistered' }}</td>
                    </tr>
                </table>
            </td>
            ...
            <td class="no-border" style="width:42%; vertical-align:top;">
                <table class="b1" style="font-size:9.5px;">
                    <tr>
                        <td style="padding:4px;" class="bold">Date:</td>
                        <td style="padding:4px;" class="right">{{ $dmy($inv->invoice_date) }}</td>
                    </tr>
                    <tr>
                        <td style="padding:4px;" class="bold">Bill No.</td>
                        <td style="padding:4px;" class="right">{{ $inv->invoice_number }}</td>
                    </tr>
                    <tr>
                        <td style="padding:4px;" class="bold">GST No.</td>
                        <td style="padding:4px;" class="right">{{ $inv->gst_no ?? ($b->gstin ?? '-') }}</td>
                    </tr>
                    <tr>
                        <td style="padding:4px;" class="bold">Transport Mode:</td>
                        <td style="padding:4px;" class="right">{{ $inv->transport_mode ?? 'By Hand' }}</td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <table class="tbl">
        ...
        <tr>
            ...
            <td class="right bold">{{ $fmt0($sumQty) }}</td> {{-- ✅ --}}
            <td colspan="3"></td>
    ...
    {{-- BELOW: reverse charge + payment + totals --}}
    <table class="no-border" style="margin-top:10px;"> {{-- ✅ spacing --}}
        <tr class="no-border">
            <td class="no-border" style="width:58%; vertical-align:top; padding-right:8px;">
                ...
                {{-- ✅ Payment Adjustment (Back Again) --}}
                <div class="tiny muted" style="margin-top:10px;">
                    <div class="bold" style="margin-bottom:4px; color:#111;">Payment Adjustment:</div>

                    <div>
                        Payment Through Credit/Debit Card :
                        {{ $payCard > 0 ? $fmt0($payCard) : '__________' }}
                        @if(!empty($cardInfo)) ({{ $cardInfo }}) @endif
                    </div>

                    <div>
                        Payment Through Online/UPI :
                        {{ $payOnline > 0 ? $fmt0($payOnline) : '__________' }}
                        @if(!empty($onlineInfo)) ({{ $onlineInfo }}) @endif
                        @if(!empty($payRow?->upi_id)) (UPI: {{ $payRow->upi_id }}) @endif
                    </div>

                    <div>
                        Payment Through CHEQUE :
                        {{ $payCheque > 0 ? $fmt0($payCheque) : '__________' }}
                        @if(!empty($chequeInfo)) ({{ $chequeInfo }}) @endif
                    </div>

                    <div>
                        Payment Through Cash :
                        {{ $payCash > 0 ? $fmt0($payCash) : '__________' }}
                    </div>
                </div>
            </td>
