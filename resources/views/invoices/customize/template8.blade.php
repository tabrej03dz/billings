@php
    ...
    $borderColor    = $setting->border_color ?? '#111827';
    $secondaryColor = $setting->secondary_color ?? '#e5e7eb';
    $lightBgColor   = $setting->light_bg_color ?? '#f3f4f6';
    ...
    $templateName = $billTemplate->name ?? $template->name ?? 'Classic Table Invoice Template';
@endphp
...
<form method="POST" action="{{ route('bill-template.customize.save', $templateId) }}">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div class="bg-white dark:bg-neutral-900 rounded-xl shadow p-5 space-y-5 h-fit">
            ...
            <div>
                <label class="text-sm font-semibold">Primary / Total Color</label>
            ...
            <div>
                <label class="text-sm font-semibold">Table Header Color</label>
                <input type="color" name="secondary_color" id="secondary_color" value="{{ $secondaryColor }}" class="w-full h-11 mt-2">
            </div>

            <div>
                <label class="text-sm font-semibold">Document Heading BG</label>
        ...
        <div class="xl:col-span-3 bg-gray-100 dark:bg-neutral-800 rounded-xl p-5 overflow-auto">
            <div id="invoicePreview"
                ...
                <table id="headerTable" style="width:100%; border-collapse:collapse;">
                    <tr>
                        <td id="logoCell" style="width:18%; border:1px solid {{ $borderColor }}; padding:10px; vertical-align:top;">
                            <div id="logoBox" style="border:1px dashed {{ $borderColor }}; padding:18px 8px; text-align:center;">LOGO</div>
                        </td>

                        <td style="width:57%; border:1px solid {{ $borderColor }}; padding:10px; vertical-align:top;">
                            <div style="font-size:22px; font-weight:700;">Real Victory Groups</div>
                            <div>73 Basement, Ekta Enclave Society, Kanpur, Uttar Pradesh - 208024</div>
                            <div>Mobile: 7753800444</div>
                            <div>Email: info@realvictorygroups.com</div>
                            <div>GSTIN: 09ABCDE1234F1Z5</div>
                        </td>

                        <td id="docBox" style="width:25%; border:1px solid {{ $borderColor }}; text-align:center; font-size:24px; font-weight:700; background:{{ $lightBgColor }};">
                            TAX<br>
                ...
                <table id="infoTable" style="width:100%; border-collapse:collapse; margin-top:12px;">
                    <tr>
                        <td style="width:50%; border:1px solid {{ $borderColor }}; padding:10px; vertical-align:top;">
                            <div class="label">Bill To</div>
                            <strong>DEMO CUSTOMER</strong><br>
                            Sample Market Road, Lucknow, Uttar Pradesh - 226001
                        </td>
                        <td style="width:25%; border:1px solid {{ $borderColor }}; padding:10px; vertical-align:top;">
                    ...
                    <tr>
                        <td style="border:1px solid {{ $borderColor }}; padding:10px;">
                ...
                <table id="itemsTable" style="width:100%; border-collapse:collapse; margin-top:12px;">
                    <thead>
                        <tr>
                ...
                <table id="bottomTable" style="width:100%; border-collapse:collapse; margin-top:14px;">
                    <tr>
                        <td style="width:60%; border:1px solid {{ $borderColor }}; padding:10px;">
                            <div class="label">Amount in Words</div>
                            <strong>Eleven Thousand Eight Hundred Rupees Only</strong>
                        ...
                        <td id="signatureBox" style="width:40%; border:1px solid {{ $borderColor }}; padding:10px; text-align:right;">
...
<script>
    ...
    function updatePreview() {
        ...
        document.getElementById('docBox').style.backgroundColor = lightBgColor.value;

        document.querySelectorAll('td, th').forEach(el => {
            if (
                el.closest('#headerTable') ||
                el.closest('#infoTable') ||
                el.closest('#itemsTable') ||
                el.closest('#totalsTable') ||
                el.closest('#bottomTable')
            ) {
        ...
        document.querySelectorAll('.label').forEach(el => {
            el.style.color = borderColor.value;
        });
        ...
        document.querySelectorAll('#totalsTable .total td').forEach(td => {
        ...
        document.getElementById('signatureBox').style.display = document.getElementById('show_signature').checked ? 'table-cell' : 'none';
